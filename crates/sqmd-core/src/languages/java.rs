...
impl LanguageChunker for JavaChunker {
        ...
        for child in tree.root_node().children(&mut cursor) {
            ...
            match kind {
                "method_declaration" | "constructor_declaration" => {
                    ...
                    if let Some(chunk) = make_chunk(source, child, file_path, "java", ct, name.as_deref(), sig.as_deref(), serde_json::Map::new()) {
                        chunks.push(chunk);
                    }
                }
                "class_declaration" | "interface_declaration" | "enum_declaration" => {
                    ...
                    all_chunks.push(make_chunk(source, child, file_path, "java", ct, name.as_deref(), None, serde_json::Map::new()));
                    ...
                    if let Some(body) = child.child_by_field_name("body") {
                        ...
                        for member in body.children(&mut body_cursor) {
                            let mk = member.kind();
                            if mk == "method_declaration" || mk == "constructor_declaration" || mk == "field_declaration" {
                                let mname = member.child_by_field_name("name")
                                    .and_then(|n| n.utf8_text(source.as_bytes()).ok())
                                    .map(|s| s.to_string());

                                let mct = match mk {
                                    "method_declaration" | "constructor_declaration" => ChunkType::Method,
                                    "field_declaration" => ChunkType::Constant,
                                    _ => ChunkType::Section,
                                };

                                all_chunks.push(make_chunk(source, member, file_path, "java", mct, mname.as_deref(), None, serde_json::Map::new()));
                            }
                        }
                    }
                ...
                "import_declaration" => {
                    if let Some(chunk) = make_chunk(source, child, file_path, "java", ChunkType::Import, None, None, serde_json::Map::new()) {
                        chunks.push(chunk);
                    }
                }
                _ => {}
    ...
    fn extract_imports(&self, source: &str) -> Vec<crate::relationships::ImportInfo> {
        ...
        for child in tree.root_node().children(&mut cursor) {
            ...
            let text = text.trim();
            if let Some(path) = text.strip_prefix("import ").and_then(|s| s.strip_suffix(';')) {
...
"#;
        ...
        let chunks = chunker.chunk(source, "UserService.java");
