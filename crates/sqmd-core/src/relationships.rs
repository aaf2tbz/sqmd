...
pub fn get_dependencies(
    db: &rusqlite::Connection,
    file_path: &str,
) -> Result<Vec<DepInfo>, Box<dyn std::error::Error>> {
    let mut stmt = db.prepare(
...
pub fn get_dependency_ids(
    ...
    let ids: Vec<i64> = stmt
        .query_map(params![chunk_id], |r| r.get(0))?
        .collect::<Result<_, _>>()?;
    Ok(ids)
}

pub fn get_dependent_ids(
    db: &rusqlite::Connection,
    chunk_id: i64,
    depth: usize,
) -> Result<Vec<i64>, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_resolve_relative_path() {
        ...
        assert!(
            results.contains(&"src/auth/utils/helpers.ts".to_string()),
            "got: {:?}",
            results
        );
    }
    ...
    fn test_resolve_parent_path() {
        ...
        assert!(
            results.contains(&"src/utils/common.ts".to_string()),
            "got: {:?}",
            results
        );
    }
    ...
    fn test_resolve_sibling() {
        ...
        let results = resolve_module_path(&db, "src/auth/login.ts", "../utils/helpers");
        assert!(
            results.contains(&"src/utils/helpers.ts".to_string()),
            "got: {:?}",
            results
        );
    }
    ...
    fn test_resolve_rust_crate_path() {
        ...
        assert!(
            !results.is_empty(),
            "should resolve crate::chunker, got: {:?}",
            results
        );
        assert!(
            results.contains(&"crates/sqmd-core/src/chunker.rs".to_string()),
            "got: {:?}",
            results
        );
    }
    ...
    fn test_get_dependency_ids_with_depth() {
        ...
        db.execute(
            "INSERT INTO relationships (source_id, target_id, rel_type) VALUES (1, 2, 'imports')",
            [],
        )
        .unwrap();
        db.execute(
            "INSERT INTO relationships (source_id, target_id, rel_type) VALUES (2, 3, 'imports')",
            [],
        )
        .unwrap();
